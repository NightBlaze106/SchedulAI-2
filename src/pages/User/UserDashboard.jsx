...
export default function UserDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const { data: teacherData } = await supabase.from('users').select('id, name').eq('role', 'teacher');
      const { data: groupData } = await supabase.rpc('get_generated_timetables');
    ...
    <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center justify-center font-sans text-slate-900">
      ...
      <Dialog open={showAuthDialog} onOpenChange={setShowAuthDialog}>
        <DialogContent className="sm:max-w-[400px] bg-white p-6 rounded-xl shadow-xl">
          ...
          <DialogFooter>
             <Button onClick={handleLoginSubmit} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white">
               Unlock Portal
             </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <div className="text-center mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-4">SchedulAI</h1>
        <p className="text-slate-500 text-lg">Select your role to access the system.</p>
      </div>
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8 px-4">
        ...
        <Card className="border-slate-200 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
          <CardHeader className="pb-4 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
            ...
            <CardTitle className="text-2xl font-bold text-slate-900">Administrator</CardTitle>
            <p className="text-sm text-slate-500 font-medium">System Configuration & Management</p>
          </CardHeader>
          <CardContent className="pt-8 pb-8 px-6">
            <p className="text-slate-600 text-sm mb-6 leading-relaxed">
              Access global settings, manage subjects, teachers, rooms, and generate new timetables.
            </p>
            <Button 
        ...
        <Card className="border-slate-200 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
          <CardHeader className="pb-4 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
            ...
            <CardTitle className="text-2xl font-bold text-slate-900">Faculty</CardTitle>
            <p className="text-sm text-slate-500 font-medium">Schedule & Classroom Management</p>
          </CardHeader>
          <CardContent className="pt-8 pb-8 px-6 space-y-6">
            <div className="space-y-2">
              <Label className="text-xs uppercase text-slate-400 font-bold tracking-wider">Select Profile</Label>
        ...
        <Card className="border-slate-200 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
          <CardHeader className="pb-4 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
            ...
            <p className="text-sm text-slate-500 font-medium">Access Timetables & Notices</p>
          </CardHeader>
          <CardContent className="pt-8 pb-8 px-6 space-y-6">
            <div className="space-y-2">
              <Label className="text-xs uppercase text-slate-400 font-bold tracking-wider">Select Class</Label>
