...
import CustomToast from "@/components/ui/CustomToast"; // <-- Import Toast
...
export default function TeacherDashboard() {
  ...
  // ... (Existing State for Notifications/Timetable) ...
  ...
  const timetableRef = useRef(null); // Ref for PDF Export
  ...
  // ... (Keep existing useEffects for data fetching) ...
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const uniqueClasses = [
          "All Classes",
          ...new Set(rawData.map((r) => r.class_name)),
        ];
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // 1. Capture original styles
      const originalOverflow = element.style.overflow;
      const originalHeight = element.style.height;
      const originalWidth = element.style.width;

      // 2. Force element to expand to full size for capture
      // This reveals the hidden scrollable content
      element.style.overflow = 'visible'; 
      element.style.height = '1200px';
      element.style.width = '1800px'; // Ensure a fixed width to prevent cramping
      
      // 3. Capture
      const canvas = await html2canvas(element, { 
        scale: 2, // High resolution
        useCORS: true,
        windowWidth: 1200 // Simulate a wide screen
      });
      ...
      // If the timetable is very long, we might need multi-page logic, 
      // but for a single week view, fitting to page is usually best.
      // If it's too tall for one page, we scale it down slightly to fit.
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
            ...
            <header className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-8 gap-4">
              <div>
                ...
                <p className="text-slate-500 text-lg">
                  Manage your classes and stay organized.
                </p>
              </div>
        ...
        {activeView === "requests" && (
          // We pass the showToast function down so the child component can trigger toasts

          <TeacherRequests
            teacherId={teacherId}
            myClasses={rawClasses}
            onToast={showToast}
          />
        )}
      </main>
